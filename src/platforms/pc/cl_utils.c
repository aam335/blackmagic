...
static void cl_help(char **argv, BMP_CL_OPTIONS_t *opt)
{
	...
	DEBUG_WARN("\t-v[1|2]\t\t: Increasing verbosity. Bitmask:\n");
	...
	DEBUG_WARN("\t-a <num>\t: Start flash operation at flash address <num>\n"
...
#define WORKSIZE 1024
		...
		if (read_file != -1)
			close(read_file);
		DEBUG_WARN("Read/Verified succeeded for %d bytes, %8.3f kiB/s\n",
