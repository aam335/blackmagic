...
{
	jtag_dev_write_ir(&jtag_proc, dtm->dev, RISCV_IR_DTMCONTROL);
...
retry:
	...
	case 3:
		riscv_dtm_reset(dtm);
		jtag_dev_write_ir(&jtag_proc, dtm->dev, RISCV_IR_DBUS);
...
void riscv_jtag_handler(jtag_dev_t *dev)
{
	...
	jtag_dev_write_ir(&jtag_proc, dev, RISCV_IR_DTMCONTROL);
	...
	jtag_dev_write_ir(&jtag_proc, dev, RISCV_IR_DBUS);
