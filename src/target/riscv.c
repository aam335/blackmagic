...
{
	jtag_dev_write_ir(&jtag_proc, dtm->dev, IR_DTMCONTROL);
	uint32_t dtmcontrol = DTMCONTROL_DBUSRESET;
	jtag_dev_shift_dr(&jtag_proc, dtm->dev, (void*)&dtmcontrol, (void*)&dtmcontrol, 32);
	DEBUG_INFO("after dbusreset: dtmcontrol = 0x%08x\n", dtmcontrol);
}
...
retry:
	...
	case 3:
		riscv_dtm_reset(dtm);
		jtag_dev_write_ir(&jtag_proc, dtm->dev, IR_DBUS);
		DEBUG_INFO("retry out %"PRIx64"\n", dbus);
		jtag_dev_shift_dr(&jtag_proc, dtm->dev,
...
static uint32_t ram_stub_result(struct riscv_dtm *dtm, int i)
{
	...
	if ((uint32_t)ex != 0) {
		DEBUG_INFO("%s exception 0x%"PRIx32"\n", __func__, (uint32_t)ex);
		dtm->exception = true;
		return 0;
	}
...
void riscv_jtag_handler(jtag_dev_t *dev)
{
	...
	DEBUG_INFO("Scanning RISC-V target! %p\n", dev);
	jtag_dev_write_ir(&jtag_proc, dev, IR_DTMCONTROL);
	jtag_dev_shift_dr(&jtag_proc, dev, (void*)&dtmcontrol, (void*)&dtmcontrol, 32);
	DEBUG_INFO("dtmcontrol = 0x%08x\n", dtmcontrol);
	uint8_t version = dtmcontrol & 0xf;

	if (version > 0) {
		DEBUG_WARN("Only DTM version 0 handle. Version is %d\n", version);
	...
	DEBUG_INFO("abits = %d\n", dtm->abits);
	DEBUG_INFO("idle = %d\n", dtm->idle);
	DEBUG_INFO("dbusstat = %d\n", (dtmcontrol >> 8) & 3);
	riscv_dtm_reset(dtm);

	jtag_dev_write_ir(&jtag_proc, dev, IR_DBUS);
	...
	for (int i = 0; i < 	dtm->dramsize + 1; i++) {
		DEBUG_INFO("DebugRAM[%d] = %08"PRIx64"\n", i,
