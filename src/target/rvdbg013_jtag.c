...
static void rvdbg_dmi_reset_jtag(RVDBGv013_DMI_t *dmi, bool hard_reset)
{
	...
	jtag_dev_write_ir(&jtag_proc, rvdbg_jtag->dev, IR_DTMCS);
	...
	jtag_dev_shift_dr(&jtag_proc, rvdbg_jtag->dev, (void*)&dtmcontrol, (void*)&dtmcontrol, 32);

	// DEBUG("after dmireset: dtmcs = 0x%08x\n", (uint32_t)dtmcontrol);

	// Switch to DMI register
	jtag_dev_write_ir(&jtag_proc, rvdbg_jtag->dev, IR_DMI);
}
...
retry:
	jtag_dev_shift_dr(&jtag_proc, rvdbg_jtag->dev, (void*)&dmi_ret, (const void*)&dmi_cmd,
	...
	switch (DMI_GET_OP(dmi_ret)) {
		case DMISTAT_OP_BUSY:
			...
			jtag_dev_shift_dr(&jtag_proc, rvdbg_jtag->dev, (void*)&dmi_ret, (const void*)&rvdbg_jtag->last_dmi,
				DMI_BASE_BIT_COUNT + dmi->abits);
...
void rvdbg013_jtag_dp_handler(jtag_dev_t *dev)
{
	...
	jtag_dev_write_ir(&jtag_proc, rvdbg_jtag->dev, IR_DTMCS);
	jtag_dev_shift_dr(&jtag_proc, rvdbg_jtag->dev, (void*)&dtmcontrol,
		(void*)&dtmcontrol, 32);
